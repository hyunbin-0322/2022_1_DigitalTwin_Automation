...
**컨베이어 벨트 구동 및 하드웨어 설계 파트**에서는 **WLKATA Mirobot Conveyor Belt** 를 **MCU(STM32F411RE)**와 **모터드라이버(MSD-224)** 를 이용하여 구동하고, 컨베이어가 동작하고 있는 상황에서 쓰레기를 일정한 위치에 고정시키기 위한 지지대를 모델링하여 출력했습니다. 컨베이어 벨트의 바이폴라 스태퍼 모터인 42HS4013A4 의 높은 토크 출력을 위해 Full-Step 제어를 선택했고, 800[Hz] 의 PWM 을 MCU 에 입력하여 240[RPM]의 속도로 구동했습니다.
